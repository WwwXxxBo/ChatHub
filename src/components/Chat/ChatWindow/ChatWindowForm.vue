...
<template>
  <a-form :model="assistant" layout="vertical">
    <!-- 助手名称 -->
    <a-form-item field="name" :label="$t('assistantList.title')">
      <a-input v-model="assistant.name" :placeholder="$t('common.pleaseEnter') + ' ' + $t('assistantList.title')
        " :max-length="30" />
    </a-form-item>
    <!-- 对话助手参数 -->
    <template v-if="assistant.type === 'chat'">
      ...
      <a-form-item field="instruction" :label="$t('assistantList.instruction')">
        <a-textarea v-model="assistant.instruction"
          :placeholder="$t('common.pleaseEnter') + $t('assistantList.instruction')" allow-clear
        ...
        <a-space direction="vertical" style="width: 100%">
          ...
          <template v-if="assistant.provider === 'Ollama'">
            ...
            <a-select v-model="assistant.model" allow-create allow-search :fallback-option="false">
              <a-option v-for="m in modelList" :value="m.name" :key="m.id">
                {{ m.name }}
              </a-option>
            </a-select>
          </template>
          <template v-else>
            <a-select v-model="assistant.model" allow-create allow-search :fallback-option="false">
              <a-option v-for="m in chatModels[assistant.provider]" :key="m.name" :value="m.value">{{ m["name"]
              }}</a-option>
            </a-select>
            ...
            <a-space v-else>
              <a-tag v-if="isSupportImage(assistant.provider, assistant.model)" color="green" bordered>
                <template #icon>
                  <icon-check />
                </template>
              ...
              <a-tag v-if="isSupportPlugin(assistant.provider, assistant.model)" color="green" bordered>
                <template #icon>
                  <icon-check />
                </template>
              ...
              <a-tag v-if="isSupportNetwork(assistant.provider, assistant.model)" color="green" bordered>
                <template #icon>
                  <icon-check />
                </template>
      ...
      <!-- 生成token限制 -->
      <a-form-item v-if="assistant.provider != 'Ollama'" field="maxTokens" :label="$t('assistantList.maxTokens')">
        <a-input-number v-model="assistant.maxTokens"
          :placeholder="$t('common.pleaseEnter') + ' ' + $t('assistantList.maxTokens')" :min="1" />
      </a-form-item>

      <!-- 输入token限制 -->
      <a-form-item field="inputMaxTokens" :label="$t('assistantList.inputMaxTokens')">
        <a-input-number v-model="assistant.inputMaxTokens"
          :placeholder="$t('common.pleaseEnter') + ' ' + $t('assistantList.inputMaxTokens')" :min="1" />
      </a-form-item>

      <!-- 上下文条数 -->
      <a-form-item field="contextSize" :label="$t('assistantList.contextSize')">
        <a-input-number v-model="assistant.contextSize"
          :placeholder="$t('common.pleaseEnter') + ' ' + $t('assistantList.contextSize')" :min="0" :max="100" />
      </a-form-item>
      ...
      <template v-if="assistant.provider === 'OpenAI'">
        ...
        <a-form-item field="speechModel" :label="$t('assistantList.speechModel')">
          <a-select v-model="assistant.speechModel" :fallback-option="false"
        ...
        <a-form-item field="speechVoice" :label="$t('assistantList.speechVoice')">
          <a-select v-model="assistant.speechVoice" :fallback-option="false"
        ...
        <a-form-item field="speechSpeed" :label="$t('assistantList.speechSpeed')">
          <a-input-number v-model="assistant.speechSpeed" :min="0.25" :max="4.0"
